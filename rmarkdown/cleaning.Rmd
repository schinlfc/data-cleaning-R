...
`dept = str_extract(Agency, ".*(ment|ice)")`.  E.g. want to extract all characters up until `ment` or `ice` (we can group in `regex` using parentheses) and then discard the rest. Replot annual salary versus hire date, color by `dept` (not yet - using ggplot)
```{r, warning=FALSE, message=FALSE}
